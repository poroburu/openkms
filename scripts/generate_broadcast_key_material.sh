...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/generate_broadcast_key_material.sh [--refresh-env [out_dir]]
...
Description:
  ...
  --refresh-env [out_dir]  Rewrite only broadcast-keys.env from existing
  solana/id.json and cosmos/private.hex (+ key.json). Fetches Cosmos REST/fees
  from chain-registry. Does not run Docker or delete key directories (safe for
  funded wallets). Default out_dir: ./.tmp/broadcast-keys

Defaults (keygen mode):
  target  = both
...
Outputs:
  <out_dir>/broadcast-keys.env (keys + chain tuning aligned with broadcast_e2e
    defaults; suitable for `generate_remote_e2e_request.sh -e <out_dir>`)
...
if [[ "$REFRESH_ENV" == "1" ]]; then
  if ((${#POSITIONAL[@]} > 1)); then
    echo "error: --refresh-env accepts at most one argument (output directory)" >&2
    usage >&2
    exit 1
  fi
  OUT_DIR="${POSITIONAL[0]:-./.tmp/broadcast-keys}"
  TARGET=""
else
...
fetch_cosmos_broadcast_env_snippet() {
...
emit_broadcast_keys_env() {
  {
    ...
    if [[ -n "$solana_seed_b64" ]]; then
      ...
      echo "# Solana cluster (same defaults as scripts/run_broadcast_e2e.sh + tests/broadcast_solana_e2e)"
      echo "OPENKMS_SOLANA_RPC_URL=https://api.devnet.solana.com"
      echo "OPENKMS_SOLANA_CHAIN_ID=devnet"
      echo "OPENKMS_SOLANA_TRANSFER_LAMPORTS=5000"
      echo
    fi
    if [[ -n "$cosmos_scalar_b64" ]]; then
      ...
      echo "# Cosmos tx sizing (same defaults as tests/broadcast_cosmos_e2e)"
      echo "OPENKMS_COSMOS_GAS_LIMIT=200000"
      echo "OPENKMS_COSMOS_TRANSFER_AMOUNT=1"
      echo
    fi
  } >"$ENV_FILE"
}

read_existing_solana_key() {
  local dir="$OUT_DIR_ABS/solana"
  local id_json="$dir/id.json"
  local addr_file="$dir/address.txt"
  if [[ ! -f "$id_json" ]]; then
    return 1
  fi
  solana_seed_b64="$(
...
PY
    ...
    solana_address=""
    echo "warning: missing $addr_file; OPENKMS_SOLANA_SIGNER_ADDRESS will be empty" >&2
  fi
  return 0
}

read_existing_cosmos_key() {
  local dir="$OUT_DIR_ABS/cosmos"
  local hex="$dir/private.hex"
  local keyjson="$dir/key.json"
  if [[ ! -f "$hex" ]]; then
    return 1
  fi
  cosmos_scalar_b64="$(
...
PY
    ...
    cosmos_address=""
    echo "warning: missing $keyjson; OPENKMS_COSMOS_SIGNER_ADDRESS will be empty" >&2
  fi
  return 0
}
...
if [[ "$REFRESH_ENV" == "1" ]]; then
  has_s=0
  has_c=0
  if read_existing_solana_key; then
    has_s=1
  fi
  if read_existing_cosmos_key; then
    has_c=1
  fi
  if [[ "$has_s" -eq 0 && "$has_c" -eq 0 ]]; then
    echo "error: --refresh-env needs solana/id.json and/or cosmos/private.hex under $OUT_DIR_ABS" >&2
    exit 1
  fi
  if [[ "$has_c" -eq 1 ]]; then
    ...
    COSMOS_CLI_IMAGE="${OPENKMS_COSMOS_CLI_IMAGE:-ghcr.io/cosmos/gaia:latest}"
    COSMOS_BROADCAST_ENV_SNIPPET=""
  fi
  emit_broadcast_keys_env
  echo "refreshed env only (key files unchanged): $ENV_FILE" >&2
  echo "solana address: ${solana_address:-}" >&2
  echo "cosmos address: ${cosmos_address:-}" >&2
  exit 0
fi
...
preflight_output_dirs() {
  ...
  echo "or refresh only broadcast-keys.env (no Docker, wallets unchanged):" >&2
  echo "  ./scripts/generate_broadcast_key_material.sh --refresh-env \"${OUT_DIR_ABS}\"" >&2
  echo "to build remote E2E request secrets from existing keys without touching wallets, run:" >&2
  echo "  ./scripts/generate_remote_e2e_request.sh -e \"${OUT_DIR_ABS}\" both" >&2
  exit 1
}
